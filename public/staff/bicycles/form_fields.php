...
<dl>
  ...
  <dd>
    ...
    <?php foreach(Bicycle::GENDERS as $gender) { ?>
      <option value="<?php echo $gender; ?>" <?php  if($bicycle->gender == $gender) {echo 'selected';}  ?> >
        <?php echo $gender; ?>
      </option>
    <?php } ?>
    </select>
...
<dl>
  <dt>Condition</dt>
  <?php 
  // var_dump(Bicycle::CONDITION_OPTIONS);
  // требовался public вместо protected для этой константы
  ?>
  <dd>
    <select name="condition_id">
      <option value=""></option>
    <?php foreach(Bicycle::CONDITION_OPTIONS as $cond_id => $cond_name) { ?>
      <option value="<?php echo $cond_id; ?>" <?php  if($bicycle->condition_id == $cond_id) {echo 'selected';}  ?> >
        <?php echo $cond_name; ?>
      </option>
    <?php } ?>
    </select>
